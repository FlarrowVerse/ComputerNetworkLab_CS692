...
student* newStudent(char *fname, char *lname, long roll) {
	...
	strcpy(record->name[0], fname);
	strcpy(record->name[1], lname);
	record->roll = roll;
	return record;
}
...
student* createNew(int i) {
	...
	printf("First Name: ");  
	fgets(record->name[0], MAX_LEN, stdin);
	...
	printf("Roll No.: "); scanf("%ld", &(record->roll)); getchar();

	return record;
}

student** createNewList(int size) {
	int i;
	student **records = (student **)calloc(size, sizeof(student *));
	for (i = 0; i < size; i++) {
...
student* parseStudent(char *str) {
	char fname[MAX_LEN], lname[MAX_LEN];
	long roll;
	sscanf(str, "%ld\t%s %s", &roll, fname, lname);
	return newStudent(fname, lname, roll);
}